#!/usr/bin/env bash
# Local (WSL) variant of cloud_jobs_wp6.sh:
...
mkdir -p logs/wp6 results/locks_local
...
worker() {
  ...
  for name in $names; do
    mkdir "results/locks_local/$name" 2>/dev/null || continue
...
for i in $(seq 1 "$nw"); do worker > "logs/wp6/local_worker_${tag}_$i.out" 2>&1 & done
wait
